...
- **[Introduction to Information Retrieval](https://nlp.stanford.edu/IR-book/)**
  ...
  Backs: classical IR — precision@k, recall@k, MAP, MRR (metric foundations) **and** TF-IDF / Okapi BM25 (sparse retrieval, Ch. 6 & 11) **and** query expansion / relevance feedback (Ch. 9) **and** near-duplicate detection — *k*-shingles, the Jaccard coefficient of shingle sets, and sketch/permutation (MinHash) estimation with union-find clustering (Ch. 19 "Web search basics", §19.6 "Near-duplicates and shingling"). *§19.6 verified at the Stanford HTML primary 2026-06-08: "Given a positive integer *k* … define the *k*-shingles of *d* to be the set of all consecutive sequences of *k* terms in *d*"; "two documents are near duplicates if the sets of shingles … are nearly the same"; the sketch is estimated over 200 random permutations and thresholded.*
...
- **[Deduplicating Training Data Makes Language Models Better](https://arxiv.org/abs/2107.06499)**
  — Lee, Ippolito, Nystrom, Zhang, Eck, Callison-Burch, Carlini (Google/UPenn). ACL 2022 (`2022.acl-long.577`). **high**.
  Backs: evidence that text-corpus duplicates are harmful and that deduplication is cheap and non-harmful — verbatim from the abstract, "over 1% of the unprompted output … is copied verbatim from the training data," dedup lets models "emit memorized text ten times less frequently and require fewer training steps to achieve the same or better accuracy," and "reduce train-test overlap, which affects over 4% of the validation set." Two released tools: **ExactSubstr** (exact substring dedup via a suffix array) and **NearDup** (approximate document-level near-dup clustering). *Scope flag: this paper is about deduplicating **LM pretraining corpora**, not a RAG corpus — it makes no claim about retrieval or top-k quality. Citing it for "dedup your RAG ingestion corpus" is our engineering inference, to be flagged in-lesson. The word "MinHash" for NearDup is from the paper body, not the abstract; the high-tier method source for shingling/MinHash is §4 Intro to IR §19.6. Verified at the ACL Anthology abstract + official author repo 2026-06-08.*
- **[Unicode Standard Annex #15 — Unicode Normalization Forms](https://www.unicode.org/reports/tr15/)**
  — The Unicode Consortium. Official standard annex (UAX #15). **high** (authoritative standard).
  Backs: Unicode normalization — "When implementations keep strings in a normalized form, they can be assured that equivalent strings have a unique binary representation." The four forms: **NFD** (canonical decomposition), **NFC** (canonical decomposition + composition), **NFKD** (compatibility decomposition), **NFKC** (compatibility decomposition + composition); canonical vs *compatibility* (the "K") equivalence. **Compatibility forms (NFKC/NFKD) are lossy**: per §1.1 the variants "may represent a visual distinction that is significant in some textual contexts … greater care is required to determine when use of a compatibility equivalent is appropriate" (e.g. `¼→1/4`, the superscript `i⁹→i9`, `①→1`, full-width CJK folding). *Verified at the primary 2026-06-08.*
- **[Character Model for the World Wide Web: String Matching](https://www.w3.org/TR/charmod-norm/)**
  — W3C. W3C Working Group Note, 11 August 2021. **high** (standards-body primary).
  Backs: **NFC as the recommended default normalization** — "Content authors SHOULD use Unicode Normalization Form C (NFC) wherever possible for content" (with the caveat "NFC is not always appropriate to the content or even available … in some languages"); transcoding legacy encodings to a consistent Unicode form (UTF-8) as the first step before comparison; and the general warning that "Normalization can remove distinctions that the users applied intentionally." *Verified at the primary 2026-06-08.*
